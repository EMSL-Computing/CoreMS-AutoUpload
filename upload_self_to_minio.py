...
def archive(filelist: list):
    zip_buffer: io.BytesIO = io.BytesIO()
    print("made zip buffer")
    with zipfile.ZipFile(zip_buffer, "w", 0, False) as dest:
        for file in filelist:
            print(Path(file).name)
            dest.write(Path(file).resolve(), arcname=Path(file).name)
    zipsize = zip_buffer.getbuffer().nbytes
    print("zip made, yielding")
    zip_buffer.seek(0)
    return zip_buffer, zipsize


def get_checksum(filename, hash_function) -> str:
    lhash_function: str = hash_function.lower()
    with open(filename, "rb") as file:
        data: bytes = file.read()
        if "md5" in lhash_function:
            readable_hash = hashlib.md5(data).hexdigest()
        elif "sha256" in lhash_function:
            readable_hash = hashlib.sha256(data).hexdigest()
    return readable_hash


def minio_upload() -> None:
    print("enter upload")
    bucket: str = "uploader-dl"
    zipname: str = "watchdog_daemon.zip"
    verfile: str = "uploader_version.txt"
    dist_dir = Path("./dist") if Path("./dist").exists() else Path(".")
    # print(dist_dir)
    fileList = [
        Path(dist_dir / "config.toml.example"),
        Path(dist_dir / "local_api.exe"),
        Path(dist_dir / "update_uploader.exe"),
        Path(dist_dir / verfile),
    ]
    print("calculate hashes")
    hashfileList = []
    for file in fileList:
        checksum = get_checksum(file, "sha256")
        newfile: Path = Path(dist_dir / (file.name + ".sha256"))
        with open(newfile, "w") as chksumfile:
            chksumfile.write(checksum)
        hashfileList.append(newfile)
    fileList.extend(hashfileList)
    print("check buckets")
    ...
    print("objects put!")
    print("compressing")
    zipdata, zipsize = archive(fileList[0:3])
    minio.put_object(
        bucket_name=bucket,
        object_name=zipname,
        data=zipdata,
